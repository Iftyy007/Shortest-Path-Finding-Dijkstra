...
def backened(src, obstacles, destination):
    def min_distance(dist, sp_set):                   # choose b/n a vertex from set of vertices connected to parent
        min = 10**10
        global min_index
        for v in range(row_col_product):                        # minimum distant adjacent vertex is chosen
    ...
    parent = [-2 for i in range(row_col_product)]                   # every vertex keep track of its parent vertex
